...
// same order
enum { TASKID_DOWN_RIGHT, TASKID_UP_LEFT };

static char* task_ids[] = {"Down Right Propagation", "Up Left Propagation", NULL};
...
unsigned max_compute_seq(unsigned nb_iter) {
	for (unsigned it = 1; it <= nb_iter; it++) {
		if ((tile_down_right(0, 0, DIM, DIM, 0) | tile_up_left(0, 0, DIM, DIM, 0)) == 0)
			return it;
	}
